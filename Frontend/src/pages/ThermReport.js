...
const THERM_POWER = 1.5

export default function ThermReport() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/api/devices/${userId}`)
      .then(res => {
        ...
        const thermoDevices = res.data.filter(device =>
          device.name.toLowerCase().includes("therm")
        )

        setDevices(thermoDevices)

        if (thermoDevices.length > 0) {
          setSelectedDevice(thermoDevices[0]._id)
        }
      })
  }, [])

  /* ================= FETCH HISTORY (THIS IS THE FIX) ================= */
/* ================= FETCH HISTORY ================= */
useEffect(() => {
...
>
  ...
  <div className="report-title-section">
    ...
    <div>
      <h2 className="report-title"> Thermostat Usage Report</h2>
...
/>
            ...
            <h4 style={{ marginTop: 20 }}>Select Thermostat Device</h4>
            <select
          ...
          <div className="light-main">
            ...
            <div className="glass-card">
              ...
              <div style={{ marginTop: 20 }}>
                <strong>Total Energy:</strong> {totalEnergy.toFixed(2)} kWh
              </div>
            </div>
                    ...
                    {events.map((event, index) => {
                      ...
                      return (
                        <tr key={index}>
                          ...
                          <td>{(hours * THERM_POWER).toFixed(2)}</td>
                        </tr>
                      )
                    })}
                  </tbody>
